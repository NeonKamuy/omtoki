...
declare module "mongoose" {
	...
	type OptionalKeys<T, K extends string> = Omit<T, K> &
	...
	type DeepNonFunctionProperties<T> = T extends Map<infer KM, infer KV> // handle map values // Maps are not scrubbed, replace below line with this once minimum TS version is 3.7: // ? Map<KM, DeepNonFunctionProperties<KV>>
		?
				| { [key: string]: DeepNonFunctionProperties<KV> }
				| [KM, KV][]
				| Map<KM, KV>
		: T extends (infer U)[]
		? (U extends object
				? {
						[V in keyof NonFunctionProperties<
							OmitReadonly<U>
						>]: U[V] extends object | undefined
	...
	type ScrubCreateDefinition<T> = DeepMapAsObject<
		DeepNonFunctionProperties<T>
	>;

	type CreateDocumentDefinition<T> = DocumentDefinition<T>;
	...
	export type CreateQuery<D> = Partial<
		HasJustId<CreateDocumentDefinition<D>> extends true
	...
	namespace Types {
		...
		class Map<V> extends global.Map<string, V> {
			...
			toObject(
				options: { flattenMaps: true } & object
			): Record<string, V>;
	...
	> extends mquery {
		...
		): Query<mongodb.FindAndModifyWriteOpResultObject<DocType>> &
			QueryHelpers;
		findOneAndUpdate(
			query: FilterQuery<DocType>,
			update: UpdateQuery<DocType>,
		...
		findOneAndUpdate(
			query: FilterQuery<DocType>,
			update: UpdateQuery<DocType>,
			options: { lean: true } & Omit<
				QueryFindOneAndUpdateOptions,
				"lean"
			>,
			callback?: (
				err: any,
				doc: DocumentDefinition<DocType>,
				res: any
			) => void
		): Query<DocumentDefinition<DocType>> & QueryHelpers;
		findOneAndUpdate(
	...
	interface Model<T extends Document, QueryHelpers = {}>
		...
		remove(
			...
			mongodb.DeleteWriteOpResultObject["result"] & {
				deletedCount?: number;
			}
		> &
			QueryHelpers;
