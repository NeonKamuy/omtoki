...
export function useUserTooltip() {
    ...
    useEffect(() => {
        document.removeEventListener(__SETTINGS__.TOOLTIP.eventOn, listenersRef.current[0]);
        document.removeEventListener(__SETTINGS__.TOOLTIP.eventOff, listenersRef.current[1]);
